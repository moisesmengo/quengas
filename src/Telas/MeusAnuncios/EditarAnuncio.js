...
import {SubirImagensBatch, atualizarRegistro, addAnuncioFirebase, ObterUsuario, obterRegistroID } from '../../Utils/Acoes'
...
export default function EditarAnuncio(props){
    ...
    useEffect(()=>{
        (
            async()=>{
                const response = await obterRegistroID("Anuncios", id)
                const {data} = response
    ...
    const editarAnuncio = async () =>{
        ...
        if(isEmpty(titulo)){
            ...
            const registrarAnuncio = await atualizarRegistro("Anuncios",id, anuncio)

            if(registrarAnuncio.statusresponse){
                setLoading(false)
                Alert.alert("Atualização concluída", 
                "O anúncio foi atualizado corretamente",
                [{
                    style: 'cancel',
                    text: 'Aceitar',
